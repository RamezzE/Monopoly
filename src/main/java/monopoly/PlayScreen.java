...
public class PlayScreen {
    ...
    public Button rollBtn, endTurnBtn, buyPropertyBtn;
    ...
    public static Queue playerQueue = new Queue(8); // queue to save the players' playing order
    ...
    private void initialize() {
       ...
       // rollBtn.setStyle("-fx-background-color: #708090; -fx-text-fill: #f5f5f5");
        rollBtn.setStyle("-fx-background-color: #708090;-fx-cursor: hand; -fx-background-radius: 2px; -fx-text-fill: #f5f5f5");
...
//        rollBtn.setTranslateY(300);
        ...
        pane.getChildren().addAll(biv,rollBtn,firstIV,secondIV,Game.back[3]);
        Game.back[3].setTranslateX(900);
        Game.back[3].setTranslateY(-300);
        balance = new Label();
        ...
        endTurnBtn.setOnAction((ActionEvent event) -> {
            ...
            if ( i >= playersCount) {
                ...
                for (int j = playersCount-1;j>=0;j--) {
                    addInfo(Game.player.get(rollSum[j][1]).getName());
                    playerQueue.enqueue(rollSum[j][1]);
                }
                ...
                for (int i = 0; i <playersCount;i++) {
                    ...
                    Game.player.get(playerQueue.front()).setTokenIndex(random);
    ...
    private void gameLoop() {
        ...
        currentPlayer = Game.player.get(playerQueue.front()); 
        ...
        propertiesOwnedText.setText(currentPlayer.getPropertiesText(Game.Board));
...
//                }
                ...
                switch(Game.Board.get(currentPlayer.getPosition()).getType()) {
                    
                    case "property":
                        Property property = (Property)Game.Board.get(currentPlayer.getPosition());
                        addInfo(playerName.getText() + " is standing on: " + property.getName());

                        if (property.getIsOwned()) {
                            if (Game.player.get(property.getOwnerID()).getName().equals(currentPlayer.getName())) { //check so player doesnt pay to himself
                                break;
                            }
                            if(!currentPlayer.pay(property.getBaseRent(),Game.player.get(property.getOwnerID()))) {
                                // player cant pay -> player loses
                                addInfo("Failed to pay rent, so " + currentPlayer.getName() + " LOSES");
                                lose = true;
                            }
                            else {
                                addInfo( playerName.getText() + " paid " + property.getBaseRent() + " rent to " + Game.player.get(property.getOwnerID()).getName());
                    ...
                    case "jail":
                        ...
                        break;
                    default:
                        addInfo(playerName.getText() + " is standing on: " + Game.Board.get(currentPlayer.getPosition()).getType());
                        break;
                }
            ...
            if (lose) {
                ...
                currentPlayer.loser(Game.Board, Game.player);
                playerQueue.dequeue();
                playersCount--;
                lose = false;
        ...
        buyPropertyBtn.setOnAction((ActionEvent event) -> {
            ...
            if (!currentPlayer.buyProperty(property)) {
                    addInfo(playerName.getText() + " couldn't buy property, not enough money! Price was " + property.getPrice());
                    
            }
            else {
                addInfo(playerName.getText() + " paid " + property.getPrice() + " & bought " + property.getName() + " (" + property.getColor() + ")" );
                if(property.isFullSet(Game.Board,Game.player)) {
    ...
    public void loadGame() {
        ...
        try{
            ...
            for (int i = 0; i<playersCount;i++) {
                if (Game.player.get(playerQueue.front()).getBalance() != Player.startingBalance || Game.player.get(playerQueue.getSize()).getPosition() != 0) {
                    error = false;
                    break;
                }
            }
            ...
            Game.menuScreen.alert.setContentText("No Game Found // Error loading previous game " + "\nChoose new game!");
